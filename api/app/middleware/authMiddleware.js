...
if (!config.okta || !config.okta.issuer || !config.okta.clientId) {
  console.error('Okta configuration is missing or incomplete. Authentication will not work properly.');
}
...
const authMiddleware = async (req, res, next) => {
  ...
  try {
    ...
    const jwt = await oktaJwtVerifier.verifyAccessToken(accessToken, config.okta.audience || 'api://default');
